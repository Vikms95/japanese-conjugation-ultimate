...
import { createArrayOfArrays, toggleBackgroundNone, toggleDisplayNone } from "./utils.js";
...
function conjugationInqueryFormatting(conjugation) {
  ...
  } else if (conjugation.type === CONJUGATION_TYPES.te || conjugation.type === CONJUGATION_TYPES.adverb) {
...
function updateCurrentWord(word) {
  ...
  const verbHtml = word.wordJSON.kanji.replaceAll("<rt>", '<span class="rt">').replaceAll("</rt>", "</span>");
...
function irregularAdjectiveConjugation(hiraganaAdjective, affirmative, polite, conjugationType) {
  if (hiraganaAdjective == "いい") {
    ...
    let conjugations = [].concat(iiConjugation(affirmative, polite, conjugationType));
...
function changeToPastPlain(c) {
  if (c == "す") {
    ...
    console.debug("Input was not real verb ending changeToPastPlain, was " + c);
  }
}
  ...
  [PARTS_OF_SPEECH.verb]: {
    [CONJUGATION_TYPES.present]: function (baseVerbText, type, affirmative, polite) {
      if (type == "irv") {
        ...
        return [masuStem(baseVerbText, type) + "ません", plainNegativeComplete(baseVerbText, type) + "です"];
      } else if (!affirmative && !polite) {
    ...
    [CONJUGATION_TYPES.past]: function (baseVerbText, type, affirmative, polite) {
      if (type == "irv") {
        ...
        return [masuStem(baseVerbText, type) + "ませんでした", plainNegativePast + "です"];
      } else if (!affirmative && !polite) {
        let plainNegative = plainNegativeComplete(baseVerbText, type);
    ...
    [CONJUGATION_TYPES.passive]: function (baseVerbText, type, affirmative, polite) {
      ...
      const verbEndingWithA = dropFinalLetter(baseVerbText) + changeUtoA(baseVerbText.charAt(baseVerbText.length - 1));
    ...
    [CONJUGATION_TYPES.causative]: function (baseVerbText, type, affirmative, polite) {
      ...
      if (type === "ru") {
        ...
        verbCausativeRoot = dropFinalLetter(baseVerbText) + changeUtoA(baseVerbText.charAt(baseVerbText.length - 1));
      }
  ...
  [PARTS_OF_SPEECH.adjective]: {
    [CONJUGATION_TYPES.present]: function (baseAdjectiveText, type, affirmative, polite) {
      if (type == "ira") {
        ...
        return [dropFinalLetter(baseAdjectiveText) + "くないです", dropFinalLetter(baseAdjectiveText) + "くありません"];
      } else if (!affirmative && polite && type == "na") {
        return [
        ...
        return [baseAdjectiveText + "じゃない", baseAdjectiveText + "ではない"];
      }
    },
    [CONJUGATION_TYPES.past]: function (baseAdjectiveText, type, affirmative, polite) {
      if (type == "ira") {
        ...
        return [baseAdjectiveText + "じゃなかった", baseAdjectiveText + "ではなかった"];
      }
    },
...
function getStandardVariationConjugations(wordJSON, partOfSpeech, conjugationType, validBaseWordSpellings) {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    if (affirmative && !polite && conjugationType === CONJUGATION_TYPES.present && wordJSON.type != "na") continue;

    conjugationObjects.push(
      getConjugation(wordJSON, partOfSpeech, conjugationType, validBaseWordSpellings, affirmative, polite)
    );
  }
...
function getConjugation(wordJSON, partOfSpeech, conjugationType, validBaseWordSpellings, affirmative, polite) {
  ...
  console.log("test valid", validBaseWordSpellings);

  validBaseWordSpellings?.forEach((baseWord) => {
    validConjugatedAnswers.push(conjugationFunction(baseWord, wordJSON.type, affirmative, polite));
  });
...
function getAllConjugations(wordJSON) {
  ...
  let validBaseWordSpellings = [toHiragana(wordJSON.kanji), toKanjiPlusHiragana(wordJSON.kanji)];

  // Words valid spelling were being prefixed with Kanji, this is a little hacky fix
  ...
  const typesWithStandardVariations = [CONJUGATION_TYPES.present, CONJUGATION_TYPES.past];
  ...
  typesWithStandardVariations.forEach((conjugationType) => {
    allConjugations.push(
      getStandardVariationConjugations(wordJSON, partOfSpeech, conjugationType, validBaseWordSpellings)
    );
  });
  ...
  if (partOfSpeech === PARTS_OF_SPEECH.verb) {
    // te
    allConjugations.push(
      getConjugation(wordJSON, partOfSpeech, CONJUGATION_TYPES.te, validBaseWordSpellings, null, null)
    );
    // volitional
    [true, false].forEach((polite) => {
      allConjugations.push(
        getConjugation(wordJSON, partOfSpeech, CONJUGATION_TYPES.volitional, validBaseWordSpellings, null, polite)
      );
    });
    // imperative
    allConjugations.push(
      getConjugation(wordJSON, partOfSpeech, CONJUGATION_TYPES.imperative, validBaseWordSpellings, null, null)
    );
  } else if (partOfSpeech === PARTS_OF_SPEECH.adjective) {
    // Add adverb
    allConjugations.push(
      getConjugation(wordJSON, partOfSpeech, CONJUGATION_TYPES.adverb, validBaseWordSpellings, null, null)
    );
  }
...
function findMinProb(currentWords) {
  ...
  for (let i = 0; i < currentWords.length; i++) {
    for (let j = 0; j < currentWords[i].length; j++) {
      min =
        currentWords[i][j].probability < min && currentWords[i][j].probability != 0
...
function updateProbabilites(currentWords, wordsRecentlySeenQueue, currentWord, currentWordWasCorrect) {
  ...
  if (currentWord.wordJSON.group) {
    ...
    currentWords[getPartOfSpeech(currentWord.wordJSON) === PARTS_OF_SPEECH.verb ? 0 : 1]
      .filter((word) => {
...
function updateStatusBoxes(word, entryText) {
  ...
  console.log("test", word.conjugation);
  ...
  if (word.conjugation.validAnswers.some((e) => e == entryText)) {
    ...
    document.getElementById("status-text").innerHTML = `Correct${
      subConjugationForm != null ? '<span class="sub-conjugation-indicator">(' + subConjugationForm + ")</span>" : ""
    }<br>${entryText} ○`;
  } else {
    ...
    document.getElementById("status-text").innerHTML =
      (entryText == "" ? "_" : entryText) + " ×<br>" + word.conjugation.validAnswers[0] + " ○";
  }
}
...
function getSubConjugationForm(word, validAnswer) {
  ...
  if (word.conjugation.type === CONJUGATION_TYPES.potential && (word.wordJSON.type === "ru" || kanjiWord === "来る")) {
...
class ConjugationApp {
  ...
  onTouchEnd(e) {
    if (this.state.activeScreen === SCREENS.results && e.target != document.getElementById("options-button")) {
      this.loadMainView();
    }
  }
  ...
  backButtonClicked(e) {
    ...
    let newMaxScoreIndex = findMaxScoreIndex(this.state.maxScoreObjects, visibleConjugationSettings);

    if (newMaxScoreIndex === -1) {
      this.state.maxScoreObjects.push(new MaxScoreObject(0, visibleConjugationSettings));
  ...
  initState(words) {
    ...
    if (
      ...
      this.state.settings = Object.assign(getDefaultAdditiveSettings(), JSON.parse(localStorage.getItem("settings")));
  ...
  applySettingsUpdateWordList() {
    const filteredWords = applyAllSettingsFilterWords(this.state.settings, this.state.completeWordList);
